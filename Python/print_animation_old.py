...
def main():
    ...
    # Prompt for 'Mimic' folder inside the selected character directory
    mimic_dir = prompt_for_directory(character_path, f"Select a 'Mimic' directory inside {character_dir}:")
    if not mimic_dir:
        return
    
    mimic_path = os.path.join(character_path, mimic_dir)

    # List all .txt files in the 'Mimic' directory
    txt_files = [f for f in os.listdir(mimic_path) if f.endswith(".txt")]
